...
function shortcutIcon(label) {
  ...
  if (l.includes("niveau") || l.includes("progression")) return "↗";
  if (l.includes("seance") || l.includes("séance")) return "🏋";
  if (l.includes("badge")) return "🏅";
  if (l.includes("stats") || l.includes("stat")) return "◌";
  if (l.includes("import")) return "⇣";
  if (l.includes("recomp") || l.includes("récomp")) return "✦";
  if (l.includes("histor")) return "☰";
  if (l.includes("reglage") || l.includes("réglage")) return "⚙";
...
function initPageShortcutBehavior() {
  ...
  const onScroll = () => {
    ...
    shortcutScrollTimer = setTimeout(() => nav.classList.remove("is-hidden"), 520);
  };
...
function renderSplash() {
  ...
  window.setTimeout(routeAfterSplash, 2850);
}

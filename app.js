...
function bottomNav() {
  if (active || !activeProfile()) return "";
  ...
  renderShell(`
    ...
    <main class="screen">
      <section class="stack">
        ...
        <h2>Qui s'entraine ?</h2>
        <p class="muted">Chaque profil a son programme, son historique et ses reglages. Radicalement plus propre que tout melanger dans une marmite numerique.</p>
      </section>
        ...
        ${state.profiles.length ? state.profiles.map(profile => `
          <article class="profile-item">
            <div>
              ...
              <p class="small">${profile.program.name} · ${(profile.logs || []).length} seance(s) terminee(s)</p>
            </div>
            <button class="btn small-btn" onclick="selectProfile('${profile.id}')">Choisir</button>
          </article>
        `).join("") : `<section class="card compact"><p class="muted">Aucun profil pour le moment.</p></section>`}
      </section>
    ...
    <main class="screen">
      <section class="card hero-card">
        <div class="stack">
          ...
          ${recommended ? `<div class="row"><span class="badge">${recommended.plannedDuration}</span><span class="badge">${recommended.recommendedDay}</span></div>` : ""}
        </div>
        ${recommended ? `<button class="btn" onclick="startSession('${recommended.id}')">Commencer</button>` : `<button class="btn" onclick="renderSessionBuilder()">Creer une seance</button>`}
      </section>
      <section class="card compact stack">
        <div class="between"><h3>Cette semaine</h3><span class="badge good">${count} / ${items.length}</span></div>
        <div class="progress-bar"><div class="progress-fill" style="width:${percent}%"></div></div>
        <div class="week-list">
          ${items.map(item => `<div class="week-item"><div><strong>${item.recommendedDay}</strong><p class="small">${item.title} - ${item.subtitle}</p></div><span class="badge ${completedThisWeek(item.id, profile) ? "good" : ""}">${completedThisWeek(item.id, profile) ? "Fait" : "A faire"}</span></div>`).join("")}
        </div>
      </section>
      ...
      <section class="session-grid">
        ${profile.program.sessions.map(session => `
          <article class="card compact session-card">
            ...
            <div class="row"><button class="btn secondary small-btn" onclick="renderSessionDetail('${session.id}')">Voir detail</button><button class="btn small-btn" onclick="startSession('${session.id}')">Commencer</button></div>
          </article>
        `).join("")}
      </section>
    </main>
...
function renderSessionDetail(sessionId) {
  ...
  renderShell(`
    ...
    <main class="screen">
      ...
      <section class="card stack-lg"><div class="stack"><span class="kicker">${session.recommendedDay} - ${session.plannedDuration}</span><h1>${session.title}</h1><p class="muted">${session.subtitle}</p></div><p>${session.description}</p><button class="btn" onclick="startSession('${session.id}')">Demarrer cette seance</button></section>
...
function startSession(sessionId) {
  const session = findSession(sessionId);
  if (!session) return;
  active = { id: uid("log"), session, mode: session.warmup.length ? "warmup" : "exercise", exerciseIndex: 0, setIndex: 0, startMs: Date.now(), completedSets: [], painNotes: [] };
...
function renderTrainingHeader(label) {
  ...
  return `<div class="training-top stack"><div class="between"><button class="btn ghost small-btn" onclick="confirmQuitSession()">Quitter</button><span class="badge">${label}</span></div><div class="progress-bar"><div class="progress-fill" style="width:${percent}%"></div></div></div>`;
}
...
function renderSettings() {
  ...
  renderShell(`
    ...
    <main class="screen">
      ...
      <section class="card compact stack warning"><h3>Zone dangereuse</h3><button class="btn warn" onclick="resetAllData()">Reinitialiser toute l'app</button><p class="small">Action locale, irreversible sans sauvegarde. Bref, le bouton rouge classique.</p></section>
      <section class="card compact stack"><h3>Version</h3><p class="small">Mon Coach v1.1.0 · PWA locale · GitHub Pages</p></section>
    </main>`);
}
...
function resetAllData() {
  ...
  state = { version: "1.1.0", introSeen: false, activeProfileId: null, profiles: [] };
  renderLanding();
}

function init() {
  registerServiceWorker();
  if (!state.introSeen) renderLanding();
  else if (!activeProfile()) renderProfileSelect();
  else renderHome();
}
